...
# 离线加载本地 CLIP 模型和处理器
local_clip_path = "/root/autodl-tmp/DP-IQA-main/clip-vit-large-patch14"
processor = CLIPProcessor.from_pretrained(local_clip_path)
model = CLIPModel.from_pretrained(local_clip_path)

# ====== 保留原始完整内容 ======
dists = ['jpeg2000 compression', 'jpeg compression', 'white noise', 'gaussian blur', 'fastfading', 'fnoise', 
         'contrast', 'lens', 'motion', 'diffusion', 'shifting',
...
type2label = {'jpeg2000 compression':0, 'jpeg compression':1, 'white noise':2, 'gaussian blur':3, 'fastfading':4, 'fnoise':5, 
              'contrast':6, 'lens':7, 'motion':8, 'diffusion':9, 'shifting':10, 'color quantization':11, 'oversaturation':12, 
              'desaturation':13, 'white with color':14, 'impulse':15, 'multiplicative':16, 'white noise with denoise':17, 
              'brighten':18, 'darken':19, 'shifting the mean':20, 'jitter':21, 'noneccentricity patch':22, 'pixelate':23,
...
# 构建文本描述
text = [f"a photo of a {c} with {d} artifacts, which is of {q} quality" 
        for q, c, d in product(qualitys, scenes, dists)]
